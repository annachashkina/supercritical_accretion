...
def ftaussa(r, dotm,alpha, rin): #r is the radius in GM/c^2
    fcorr=1.-sqrt(old_div(rin,r))
    fcorr=old_div((fcorr+fabs(fcorr)),2.)
...
def hssb(r, mdot,alpha, rin):
    fcorr=1.-sqrt(old_div(rin,r))
    fcorr=old_div((fcorr+fabs(fcorr)),2.)
    h=0.00747*alpha**(-0.1)*mdot**0.2*r**(old_div(1.,20.))*fcorr**0.2
 #   print h
#    jji=raw_input()
    return 0.00747*alpha**(-0.1)*mdot**0.2*r**(old_div(21.,20.))*fcorr**0.2*sqrt(5.)
