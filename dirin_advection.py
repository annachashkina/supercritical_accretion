...
def domega(omega,r,tau,mdot,wrf,tc):
    return -0.5*omega/r+alpha*tau*(omega**2.-1.)/(2.*mdot*r**0.5)+512.*pi/9.*epsilon*(psi-1.)*omega*tc**4.*r**2./tau/mdot+wrf*r**0.5/mdot

def dmdot(r,tc,tau):
    return 512.*pi/9.*epsilon*r**2*tc**4./tau
...
#    jjii=raw_input()
    return 8.*chi/5.*alpha*tau*tc/wrf

def comegaga(tc,tau,r,wrf):
    return 8.*pi*tc**4./((tvert*tau+1.)*r*wrf)*varold+varnew*512.*pi/9.*tc**4./(tau*r*wrf)
 #   return 512.*pi/9.*tc**4./(tau*r*wrf)

def ctau(tau,tc,wrf,r,mdot):
    bet=beta(tau,tc,wrf)
    return 15./16.*mdot/(alpha*r*r*tau)*(104.*bet/15.-17.*pi*bet/32.-104./15.)

def cwrf(tau,tc,wrf,r,mdot):
    bet=beta(tau,tc,wrf)
    return 15./16.*mdot/(alpha*r*r*tau)*(-8.*bet/3.+5.*pi*bet/32.+8./3.)

def ctemp(tau,tc,wrf,r,mdot):
    bet=beta(tau,tc,wrf)
    return 15./16.*mdot/(alpha*r*r*tau)*(-64.*bet/5.+9.*pi*bet/16.+64./5.)
...
#            mdot=mdotglobal
        ...
        if(r<(rlast/(1.+drout))):
            ...
            do.append(abs(domega(omega1,r1,tau1,mdot1,wrf1,tc1))/omega1)
...
#    mdot=mdotglobal
 ...
 #       Qrad=128./9.*tc1**4./tau1
        ...
        Stau=104.*bet/15.-17.*pi*bet/32.-104./15.
        Swrf=-8.*bet/3.+5.*pi*bet/32.+8./3.
        Stemp=-64.*bet/5.+9.*pi*bet/16.+64./5.
        ...
        Qrad=128./9.*tc1**4.*4.*pi/(tau1)

 #       Qrad=2.*tc1**4./(tvert*tau1+1.)

        Qadv=-15./(16.)*mdot1*wrf1/(alpha*r1*tau1)*(part1+part2+part3+part4)

        Qplus=wrf1*r1**(-1./2.)*(domega(omega1,r1,tau1,mdot1,wrf1,tc1)-3./2.*omega1/r1)

 #       Qrad=128./9.*tc1**4./(tau1)*4.67914e+27

 #       Qrad=4.*pi*2.*tc1**4./(tvert*tau1+1.)*3.29056e+42*r**3.*varold+4.*pi*varnew*128./9.*tc1**4./(tau1)*3.29056e+42*r**3.
#
#        Qadv=-15./(16)*1.30324e+26/0.35*mdot1*wrf1*r1*r1/(alpha*tau1)*(part1+part2+part3+part4)
#
#        Qplus=-1.30324e+26/0.35*wrf1*r1**(-1./2.)*r1**3.*(domega(omega1,r1,tau1,mdot1,wrf1,tc1)-3./2.*omega1/r1)
        ...
        if(r<(rlast/(1.+drout))):
            ...
            hssb.append(ss.hssb(r,mdot, alpha, rin))
            mdot123.append(mdot)
            f.write(str(r/rin)+' '+str(omega)+' '+str(h/r)+' '+str(tau)+' '+str(tc)+' '+str(wrf)+' '+str(ss.ftaussa(r,mdot, alpha, rin))+' '+str(ss.ftaussb(r,mdot, alpha, rin))+' '+str(Qadv)+' '+str(Qrad)+' '+str(Qplus)+' '+str(ss.hssa(mdot,r, rin))+' '+str(ss.hssb(r,mdot, alpha, rin))+'\n')
    ...
    plot(rar*rin*rrin*206746., p3ar*0.+0.43, color='blue',label='P4')
    ylim(0.,7.)
    ylabel('$P1,2,3,4$')
    xlabel('$r$')
    legend()
...
#    xscale('log')
 ...
 #   plt.clf()
#    plot(tauar, bbeta*0.+1./3., color='r')
 #   plot(tauar, bbeta*0.+4./3., color='b')
#    plot(tauar, bbeta, ',',color='b')
    ...
    print "rastr: omega = "+str(omega)+"; h = "+str(h)+"; htormax = "+str(htormax)+"; mdotin = "+str(mdot)+"; wrf = "+str(wrf)+"\n"
    print "rastr: omega_in = "+str(omega_in)+"; h_in = "+str(h_in)+" ; wrf_in = "+str(wrf)+"\n"

    return omega, h, tau, htormax, mdot, wrf
...
def ordiv_smart(newmu, newmdot, newps):
    ...
    print 'here'
    co=scipy.optimize.root(vrapper,(0.81,0.96),method='hybr',jac=None,tol=None,callback=None,options=None)
    print co
