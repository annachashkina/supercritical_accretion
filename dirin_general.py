...
#        tau1=ftau(wrf1,r1,tc1)
        ...
        if((r*r)>(factor*9.*tau/(4.*64.*pi*tc**4.))):
...
#        h1=fh(wrf1,r1,tc1)
 ...
 #       tau=ftau(wrf,r,tc)
        ...
        hprev=h
##        h=fh(wrf,r,tc)
        if((r1*r1)>(factor*9.*tau1/(4.*64.*pi*tc1**4.))):
...
#    mdot=mdotglobal
 ...
 #   tau=ftau(wrf,r,tc)
    ...
    while(r>=rin):
        ...
        if((r1*r1)>(factor*9.*tau1/(4.*64.*pi*tc**4.))):
 ...
 #       Qrad=128./9.*tc1**4./tau1
        ...
        if((r*r)>(factor*9.*tau1/(4.*64.*pi*tc**4.))):
            mdot=mdot+dmdot(r1,tc1,tau1)*dr
...
def experimental(newmu,newps):
    ...
    for i in arange(20):
        newmdot=newmdot+delta
        b.parset(newmu=newmu, newmdot=newmdot,newps=newps,neweta=0.,newalpha=0.1)
        parset(newmu=newmu, newmdot=newmdot,newps=newps,neweta=0.,newalpha=0.1)
        x=ordiv_smart(newmu, newmdot, newps)
        
 #       co=scipy.optimize.root(vrapper,(xiest,qeqest),method='hybr',jac=None,tol=None,callback=None,options=None)
 #       print co
 #       jiuj=raw_input()
        xiest=x[0]
        qeqest=x[1]
        ...
        qeqq.append(qeqest)
        md.append(newmdot)
        f.write(str(newmdot)+' '+str(xiest)+' '+str(qeqest)+'\n')
...
def num_mod():
    global mdotglobal, mu, ps

    fname='rmm_exp.txt'
    fmine=open(fname,'r')
    s=str.split(str.strip(fmine.readline()))
    while(s):
        p1.append(s[0]) # in peq units
        p2.append(s[1])
        p3.append(s[2])
        p4.append(s[3])
        s=str.split(str.strip(fmine.readline()))
    ...
    mu=asarray(p1,dtype=double)
    dotm=asarray(p2,dtype=double)
    xi=asarray(p3,dtype=double)
    qeq=asarray(p4,dtype=double)
